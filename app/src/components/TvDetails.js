...
class TvDetails extends Component {
    ...
    async componentDidMount() {
        //movie details
        const response = await fetch('https://api.themoviedb.org/3/tv/'+ this.props.movie_id +'?api_key=04a30d5152c77afe4a81783d17e20316&language=hu-HU');
        const movie = await response.json();

        //main characters and actors
        const cresp = await fetch('https://api.themoviedb.org/3/tv/'+ this.props.movie_id +'/credits?api_key=04a30d5152c77afe4a81783d17e20316');
        const creds = await cresp.json();

        //Age certifications
        const certification = await fetch('https://api.themoviedb.org/3/tv/'+ this.props.movie_id +'/release_dates?api_key=04a30d5152c77afe4a81783d17e20316');
        const cert = await certification.json();

        //Movie videos, trailers
        const trailers = await fetch('https://api.themoviedb.org/3/tv/'+ this.props.movie_id +'/videos?api_key=04a30d5152c77afe4a81783d17e20316&language=hu-HU');
    ...
    render() {
        const {movie, genres, companies, isLoading, image_pre,image_post,credits, showHide,videos, video_pre} = this.state;
                        ...
                        <div className="container" style={{maxWidth: "1300px", padding: "30px 40px"}}>
                            <div className="row">
                                ...
                                <div className="col-sm-9 MovieDetailsRight">
                                    <div>
                                        <h2 style={{fontWeight:"700"}}>{movie.name}</h2>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <FontAwesomeIcon icon={faCircle} style={{color:"#fff"}} size="xs"/>
                                        {this.minutesToHours(movie.episode_run_time[0])}
